...
def main():
    ...
    parser.add_argument('-bsm_wvln', '--bsm_operating_wavelength', type=int, default=1389,
                        help='photon wavelength the BSM receives after Rb conversion') #think this is good after conversion recieve this wavelength
    ...
    parser.add_argument('-image_loss', '--imaginglossprob', type=float, default=0.095, #TODO CHECK WITH CAITAO from the calcs I ran with data from paper
   ...
   # parser.add_argument('-max_cycles', '--maxcyclesbeforeloss', type=int, default=200,
                        #help='maximum excitation cycles before forcing Rb atom loss') 
    ...
    for bsm_node in network_topo.get_nodes_by_type(YbRouterNetTopo.BSM_NODE):
        ...
        #Hayden did similar here but with the parser, I find easier to just set 0 and max efficency here
        # Rb already converts inside memory.py--> so we pass through here to keep compatibility with exisiting model code so on ERROR WITH OUT THIS PASS
        for qfc in bsm_node.get_components_by_type("QFC"):
            qfc.input_wvln = bsm_operating_wavelength
