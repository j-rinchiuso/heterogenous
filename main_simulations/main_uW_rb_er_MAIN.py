...
def configure_bsm_nodes(network_topo, detector_efficiency, detector_dark_count, 
                        er_wavelength, uw_output_wavelength):
    bsm_hardware_name = "HetTimeBinBSM"

    bsm_qfc_wavelengths = {
        "BSM_0_1": {
            "QFC0": (uw_output_wavelength, uw_output_wavelength),
            "QFC1": (uw_output_wavelength, uw_output_wavelength),
        },
        "BSM_1_2": {
            "QFC1": (er_wavelength, er_wavelength),
            "QFC2": (er_wavelength, er_wavelength),
        },
    }
...
def configure_router_nodes(network_topo, rb_photon_collection_efficiency, er_photon_collection_efficiency, 
                           er_bin_width, er_bin_separation, er_coherence_time, transducer_efficiency, 
                           transducer_noise, transmon_coherence_time, uw_output_wavelength, er_wavelength, 
                           imaging_loss_probability, image_interval, cooling_time, loading_time, 
                           eta1_converter_efficiency, eta2_converter_efficiency, converter_noise, 
                           bin_width, bin_separation):
    ...
    for node in network_topo.get_nodes_by_type(YbRouterNetTopo.QUANTUM_ROUTER):
        ...
        if node.memo_type == "uW":
            ...
            node.enable_directional_conversion({
                "BSM_0_1": {
                    "input_wavelength": 780,
                    "output_wavelength": uw_output_wavelength,
                    "amzi_efficiency": eta2_converter_efficiency,
                    "qfc_efficiency": eta1_converter_efficiency,
                    "qfc_noise": converter_noise,
                    "bin_width": bin_width,
                    "bin_separation": bin_separation,
                },
                "BSM_1_2": {
                    "input_wavelength": 780,
                    "output_wavelength": er_wavelength,
                    "amzi_efficiency": eta2_converter_efficiency,
                    "qfc_efficiency": eta1_converter_efficiency,
                    "qfc_noise": converter_noise,
                    "bin_width": bin_width,
            ...
            for mem in node.get_components_by_type(MemoryArray)[0].memories:
                mem.efficiency = rb_photon_collection_efficiency
                mem.original_memory_efficiency = rb_photon_collection_efficiency
                mem.imaging_loss_prob = imaging_loss_probability
...
def main():
    ...
    parser.add_argument("-config", "--configfile", type=str, default="config/line_3_uW_Rb_Er.json", help="network config file")
    parser.add_argument("-log", "--logfile", type=str, default="tmp/uW_rb_er.log", help="log file path")
    parser.add_argument("-rb_pce", "--rbphotoncollectionefficiency", type=float, default=0.5, help="Rb photon collection efficiency")
    ...
    parser.add_argument("-eta1", "--eta1converterefficiency", type=float, default=0.99, help="Rb frequency conversion efficiency")
    parser.add_argument("-eta2", "--eta2converterefficiency", type=float, default=0.98, help="Rb AMZI format conversion efficiency")
    parser.add_argument("-converter_noise", "--converternoise", type=float, default=0.005, help="Rb frequency conversion noise probability")
    ...
    parser.add_argument("-image_loss", "--imaginglossprob", type=float, default=0.095, help="probability imaging finds the Rb atom lost")
    parser.add_argument("-image_interval", "--imageinterval", type=int, default=20, help="number of Rb excitation cycles between imaging checks")
    parser.add_argument("-cool", "--coolingtime", type=int, default=1_000_000_000, help="Rb cooling time in ps")
    parser.add_argument("-load", "--loadingtime", type=int, default=10_000_000, help="Rb loading time in ps")
    parser.add_argument("-bwidth", "--binwidth", type=int, default=520_000, help="converted Rb time-bin width")
    parser.add_argument("-bsep", "--binseparation", type=int, default=2_800_000, help="converted Rb early-late bin separation")
    ...
    configure_bsm_nodes(network_topo, args.detectorefficiency, args.detectordarkcount, 
                        args.erphotonwavelength, args.uw_output_wavelength)
    
    tl = network_topo.get_timeline()

    log.set_logger(__name__, tl, args.logfile)
    log.set_logger_level("WARNING")
    modules = ["main_uW_rb_er_MAIN"]
    for module in modules:
        log.track_module(module)
    
    name_to_app = configure_router_nodes(network_topo, args.rbphotoncollectionefficiency, 
                                        args.erphotoncollectionefficiency, args.er_binwidth, 
                                        args.er_binseparation, args.er_coherence_time, args.transducer_efficiency, 
                                        args.transducer_noise, args.transmon_coherence_time, args.uw_output_wavelength, 
                                        args.erphotonwavelength, 
                                        args.imaginglossprob, args.imageinterval, args.coolingtime, args.loadingtime, 
                                        args.eta1converterefficiency, args.eta2converterefficiency, args.converternoise, 
                                        args.binwidth, args.binseparation)
    ...
    for i in range(args.numtrials):
        ...
        app.basis = basis
        log.logger.warning(f"Starting uW-Rb-Er EG attempt {i + 1} at {tl.time}.")
        tl.run()
