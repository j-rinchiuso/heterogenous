...
def main():
    ...
    parser.add_argument("-pce", "--photoncollectionefficiency", type=float, default=0.5,
                        help="photon collection efficiency into fiber")
    ...
    parser.add_argument("-log", "--logfile", type=str, default="tmp/rb_yb.log",
                        help="log file path")

    args = parser.parse_args()

    network_topo = YbRouterNetTopo("config/linearRbYb.json")
    ...
    for bsm_node in network_topo.get_nodes_by_type(YbRouterNetTopo.BSM_NODE):
        ...
        for qfc in bsm_node.get_components_by_type("QFC"):
            if qfc.name.endswith("QFC0"):
                ...
                qfc.input_wvln = args.ybphotonwavelength
            else:
    ...
    log.track_module("main_rb_yb_EG_sim")
    ...
    for node in network_topo.get_nodes_by_type(YbRouterNetTopo.QUANTUM_ROUTER):
        ...
        if node.memo_type == "Rb":
            ...
            for mem in node.get_components_by_type(MemoryArray)[0].memories:
                mem.efficiency = args.photoncollectionefficiency
                mem.original_memory_efficiency = args.photoncollectionefficiency
                mem.set_wavelength(args.ybphotonwavelength)

        else:
            raise ValueError(f"Only functional for Rb and Yb memories, got {node.memo_type}.")
    ...
    for i in range(args.numtrials):
        ...
        log.logger.warning(f"Starting Rb-Yb EG attempt {i + 1} at {tl.time}.")
        tl.run()
    ...
    log.logger.warning(f"yb_wavelength:{args.ybphotonwavelength}")
    log.logger.warning(f"rb_telecom_wavelength:{args.rb_telecom_wavelength}")
    ...
    print(f"After {args.numtrials} successful Rb-Yb entanglement attempts, calculated fidelity={fid}")
