"""Plot the Mw-Yb-Er efficiency-grid precise fidelity as a heatmap."""
...
DEFAULT_OUTPUT = Path("tmp/mw_yb_er_efficiency_fidelity_heatmap.png")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot the Mw-Yb-Er efficiency-grid precise fidelity."
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
    ...
    parser.add_argument(
        "--annotate",
        action="store_true",
        help="Write the precise fidelity inside each heatmap cell.",
    )
    return parser.parse_args()


def read_precise_fidelity(log_file: Path) -> float:
    """Read the final calculated precise fidelity from a log."""
...
def load_fidelity_grid(data_dir: Path) -> np.ndarray:
    """Return Mw-efficiency rows and Er-PCE columns of precise fidelity."""
    ...
    for log_file in data_dir.glob("*.log"):
        ...
        ] = read_precise_fidelity(log_file)
...
def plot_heatmap(grid: np.ndarray, output: Path, annotate: bool) -> None:
    ...
    ax.set_title("Mw-Yb-Er Fidelity")
...
def main() -> None:
    args = parse_args()
    grid = load_fidelity_grid(args.data_dir)
