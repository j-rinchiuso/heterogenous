...
def make_plot(data_dir, output_filename):
    cooling_points = collect_series(
        glob.glob(f"{data_dir}/rb_cooling_time/cooling_time=*.log"),
        x_key="cooling_time_ms",
    )
    pce_points = collect_series(
        glob.glob(f"{data_dir}/rb_pce/pce=*.log"),
        filename_pattern=r"pce=([0-9.]+)\.log",
    )
    detector_points = collect_series(
        glob.glob(f"{data_dir}/rb_detector_eff/detector_eff=*.log"),
...
make_plot("data_in_paper_ext", "data_in_paper_ext/rb_sweeps_10microsec.png")
make_plot("data_in_paper_ext_300ms", "data_in_paper_ext_300ms/rb_sweeps_300ms.png")
